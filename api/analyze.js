export default async function handler(req, res) {
  ...
  try {
    ...
    if (!apiRes.ok) {
      ...
      res.setHeader('Access-Control-Allow-Origin','*').setHeader('Content-Type','application/json').status(502).send(JSON.stringify({ error: 'Claude API error ' + apiRes.status + ': ' + errText.slice(0, 300)); return) };
    }
    ...
    if (start === -1 || end === -1) {
      res.setHeader('Access-Control-Allow-Origin','*').setHeader('Content-Type','application/json').status(500).send(JSON.stringify({ error: 'No JSON in response. Got: ' + clean.slice(0, 200)); return) };
    }
    ...
    try {
      ...
      res.setHeader('Access-Control-Allow-Origin','*').setHeader('Content-Type','application/json').status(500).send(JSON.stringify({ error: 'JSON parse failed: ' + parseErr.message + ' | Raw: ' + clean.slice(0, 200)); return) };
    }

    // Validate required fields
    if (!analysis.recommendation || !analysis.confidence) {
      res.setHeader('Access-Control-Allow-Origin','*').setHeader('Content-Type','application/json').status(500).send(JSON.stringify({ error: 'Incomplete analysis response', received: Object.keys(analysis)); return) };
    }

    res.setHeader('Access-Control-Allow-Origin','*').setHeader('Content-Type','application/json').status(200).send(JSON.stringify(analysis)); return;
